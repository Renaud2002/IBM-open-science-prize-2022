...
#import brute_force_optimization as bforce
#import optimization as opt
import copy as cp
#import find_normalizer2 as fn
#import using_qecc as q
...
def choose_basis_vector(nullspace):
    ...
    bf = bforce.brute_force_optimize(nullspace)
    # if bf is None:
    #     return minimize_cnot(nullspace)
    return bf
...
# def reduce_column_optimized(basis, qmap, x,z, X,Z,S,U):
...
#         if basis[i] == 1 and basis[i+ntmp] == 1:

#             X,Z,S,U = sgate(X,Z,S,U,qmap[i])
#             x,z,s,u = sgate(x,z,s,u,i)

#             X,Z,S,U = hgate(X,Z,S,U,qmap[i])
#             x,z,s,u = hgate(x,z,s,u,i)
...
#             X,Z,S,U = cxgate(X,Z,S,U,qmap[ctrl], qmap[trgt])
#             x,z,s,u = cxgate(x,z,s,u, ctrl, trgt)


#     # the ones that are different: step2
#     alist = []
#     blist = []

#     for i in range(ntmp):

#         if basis[i] == 1 and basis[i+ntmp] == 0:
#             alist.append(i)
#             pivots.append(i)

#         if basis[i] == 0 and basis[i+ntmp] == 1:
#             blist.append(i)
#             pivots.append(i)
...
def main_diagonalizer(pstrings):
    ...
    for bb in range(n):
        ...
        #basis = bfo.search_normalizer(x,z)
        basis = bfo.brute_force_optimize(nullspace)
        #basis = opt.search_for_vector(x,z)
        #basis = bfo.search_normalizer(nullspace)
        #basis = q.minimize_weight(x,z,nullspace)
